...
class model_creator ():
    def __init__(self, cnn_params, dense_params, input_size=(100,100,3), optimizer='adam',
                 loss='categorical_crossentropy', metrics=['accuracy'], callbacks=[], filepath=None):
        ...
        if (filepath == None) or (not os.path.isdir(filepath)):
            self.filepath = os.path.join(os.getcwd(),'best_weights.hdf5')
        else: self.filepath = os.path.join(filepath,'best_weights.hdf5')
        
        
    def CNN_layer(self, X, filters, kernel=(3, 3), stride=(2, 2), padding='valid', activation='relu', kernel_initializer='glorot_uniform', mp_kernel=(2,2), mp_stride=(2,2), name = ''):
        '''
    ...
    def compile_Model(self):
        ...
        #try:
            if self.callbacks:
                checkpoint = ModelCheckpoint(self.filepath, monitor='val_loss', verbose=0, 
